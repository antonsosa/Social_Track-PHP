...
$pdf->SetFont('Arial', 'B', 19); // Tipo de fuente, negrita(B-I-U-BIU), tamaño de texto
...
$pdf->SetFont('Arial', 'B', 12); // Cambiar el tamaño de la fuente a 12
$pdf->SetXY(68, 10); // Establecer la posición X y Y para el siguiente elemento
$pdf->Cell(160, 45, utf8_decode('Reporte General por Laboratorio'), 0, 1, 'C');

$pdf->Ln(10); // Salto de línea
$pdf->SetTextColor(103); // Color del texto
...
// Tabla 1 - Información del Laboratorio
$sql_laboratorio = "SELECT * FROM vista_horas_lab WHERE Laboratorio = '$busqueda' AND Ciclo = '$ciclo'";
...
// Tabla 2 - Detalles de las horas
$sql_horas = "SELECT * FROM vista_horas_lab WHERE Laboratorio = '$busqueda' AND Ciclo = '$ciclo'";
$result_horas = mysqli_query($conexion, $sql_horas);
$contadorId = 0;

$posicionX = 84; // Posición horizontal inicial
$posicionY = 56; // Posición vertical inicial
$desplazamientoY = 7; // Desplazamiento vertical

if ($result_horas->num_rows > 0) {
    $pdf->SetXY(84, 49); // Establecer la posición X y Y para el siguiente elemento
    $pdf->SetTextColor(0, 0, 0); // Color del texto
    $pdf->SetDrawColor(163, 163, 163); // Color del borde
    $pdf->SetFont('Arial', 'B', 8);
    ...
    $pdf->Cell(25, 7, utf8_decode('TOTAL HORAS'), 1, 0, 'C');
    $pdf->Ln(7);
    ...
    while ($row = $result_horas->fetch_assoc()) {
        ...
        $pdf->Cell(10, 7, $contadorId, 1, 0, 'C'); // Campo 'Id'
        $pdf->Cell(20, 7, utf8_decode($row['Carnet']), 1, 0, 'C'); // Campo 'Carnet'
        $pdf->Cell(70, 7, utf8_decode($row['Nombre']), 1, 0, 'L'); // Campo 'Nombre'
        $pdf->Cell(25, 7, utf8_decode($row['total_horas_lab']), 1, 1, 'C'); // Campo 'Total Horas'

        $posicionY += $desplazamientoY; // Actualizar la posición vertical para la próxima fila
    }
} else {
