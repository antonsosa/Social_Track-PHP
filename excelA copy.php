<?php
    ...
    $sheet->setCellValue('C11', 'Carnet');
    $sheet->setCellValue('D11', 'Nombre');
    $sheet->setCellValue('E11', 'Entrada');
    $sheet->setCellValue('F11', 'Salida');
    $sheet->setCellValue('G11', 'Actividad');
    $sheet->setCellValue('H11', 'Laboratorio');
    $sheet->setCellValue('I11', 'Encargado');
    $sheet->setCellValue('J11', 'Ciclo');
    $sheet->setCellValue('K11', 'Horas');
    ...
    $headerStyle2 = [
        ...
        $sheet->getStyle('B11:K11')->applyFromArray($headerStyle2);
    
        // Obtener datos de la base de datos para la tabla 2
        $ciclo = $_GET['ciclo'];
        $busqueda = $_GET['busqueda'];
        $sql_horas2 = "SELECT * FROM vista_horas_alumnos WHERE Carnet = '$busqueda' AND Ciclo = '$ciclo'";
        ...
        if($result_horas2->num_rows > 0) {
            ...
            while($row = $result_horas2->fetch_assoc()) {
                ...
                $sheet->setCellValue('C'.$rowIndex, $row['Carnet']);
                $sheet->setCellValue('D'.$rowIndex, $row['Nombre']);
                $sheet->setCellValue('E'.$rowIndex, $row['Entrada']);
                $sheet->setCellValue('F'.$rowIndex, $row['Salida']);
                $sheet->setCellValue('G'.$rowIndex, $row['Actividad']);
                $sheet->setCellValue('H'.$rowIndex, $row['Laboratorio']);
                $sheet->setCellValue('I'.$rowIndex, $row['Encargado']);
                $sheet->setCellValue('J'.$rowIndex, $row['Ciclo']);
                $sheet->setCellValue('K'.$rowIndex, $row['Total_Horas']);
                $rowIndex++; 
                $contador++;
            }
    
            // Aplicar bordes a todas las celdas de la tabla 2
            $dataRange2 = 'B11:K' . ($rowIndex - 1);
