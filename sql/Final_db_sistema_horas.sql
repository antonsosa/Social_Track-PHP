...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `agregar_encargado` (IN `p_nombres` VARCHAR(50), IN `p_apellidos` VARCHAR(50), IN `p_email` VARCHAR(100), IN `p_contraseña` VARCHAR(255), IN `p_numero_empleado` VARCHAR(15), IN `p_dui` VARCHAR(10), IN `p_tipo_usuario` INT, IN `p_laboratorios_asignados` VARCHAR(1000))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `agregar_laboratorio` (IN `nombre_lab` VARCHAR(255), IN `telefono_lab` VARCHAR(255), IN `ubicacion_lab` VARCHAR(255), IN `Denominacion_laboratorio` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `alumnos_marcacion` (IN `p_carnet` VARCHAR(12))   BEGIN
    SELECT 
        `a`.`id_alumno` AS `Id`,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CambiarContraseñaEncargado` (IN `p_id_encargado` INT, IN `p_contraseña_actual` VARCHAR(255), IN `p_nueva_contraseña` VARCHAR(255), OUT `resultado` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarAlumno` (IN `nombres` VARCHAR(100), IN `apellidos` VARCHAR(100), IN `carnet` VARCHAR(20), IN `dui` VARCHAR(20), IN `telefono` VARCHAR(20), IN `email` VARCHAR(100), IN `contacto_emergencia` VARCHAR(100), IN `telefono_emergencia` VARCHAR(20), IN `carrera` VARCHAR(100))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarCiclo` (IN `nombre_ciclo` VARCHAR(255))   BEGIN
    INSERT INTO ciclos (nombre_ciclo) VALUES (nombre_ciclo);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarCicloRecibido` (IN `p_ciclo_actual` VARCHAR(255))   BEGIN
    DECLARE v_count INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Insertar_Horas` (IN `entrada_hora_val` DATETIME, IN `salida_hora_val` DATETIME, IN `actividad_val` VARCHAR(100), IN `id_alumno_hora_val` VARCHAR(50), IN `id_encargado_val` VARCHAR(50), IN `id_laboratorio_val` VARCHAR(50), IN `nombre_laboratorio_val` VARCHAR(50), IN `nombre_ciclo_val` VARCHAR(50))   BEGIN
    INSERT INTO horas (
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificar_laboratorio` (IN `p_id_laboratorio` INT, IN `p_nombre_laboratorio` VARCHAR(50), IN `p_telefono_laboratorio` VARCHAR(9), IN `p_ubicacion_laboratorio` VARCHAR(100), IN `p_denominacion_laboratorio` VARCHAR(200))   BEGIN
    UPDATE laboratorios
    SET 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtenerNombresCiclos` ()   BEGIN
    SELECT DISTINCT `nombre_ciclo` FROM `ciclos`;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtenerRegistrosLaboratorios` (IN `encargado_id` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `registros_abiertos` (IN `id_encargado` INT)   BEGIN
    SELECT *
    FROM horas
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `registros_abiertos2` ()   BEGIN
    SELECT *
    FROM horas
    WHERE (salida_hora IS NULL OR salida_hora = '');
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `registros_horas` (IN `p_busqueda` VARCHAR(255), IN `p_ciclo` VARCHAR(255))   BEGIN
    SELECT * FROM horas
    WHERE id_alumno_hora = p_busqueda AND nombre_ciclo_hora = p_ciclo;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Update_Horas` (IN `p_id_registro` INT, IN `p_carnet` VARCHAR(12), IN `p_hora_entrada` TIMESTAMP, IN `p_actividad` VARCHAR(100), IN `p_laboratorio` INT, IN `p_hora_salida` DATETIME, OUT `p_result` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Update_Horas2` (IN `p_id_registro` INT, IN `p_carnet` VARCHAR(12), IN `p_hora_entrada` TIMESTAMP, IN `p_actividad` VARCHAR(100), IN `p_laboratorio` INT, IN `p_hora_salida` DATETIME)   BEGIN
    UPDATE horas
    SET 
        id_alumno_hora = p_carnet,
        entrada_hora = p_hora_entrada,
