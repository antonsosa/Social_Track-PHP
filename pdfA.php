<?php
    ...
    $sql_alumno = "SELECT * FROM vista_horas_ciclo WHERE Carnet = '$busqueda' AND Ciclo = '$ciclo'";
    ...
    if ($row) {
        $pdf->SetXY(79, 35); // Establecer la posición X y Y para el siguiente elemento
        ...
        $pdf->SetXY(79, 42); // Establecer la posición X y Y para el siguiente elemento
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(30, 7, utf8_decode($row['Carnet']), 1, 0, 'C'); // Campo 'Id'
        $pdf->Cell(50, 7, utf8_decode($row['Nombre']), 1, 0, 'C'); // Campo 'Entrada'
        $pdf->Cell(30, 7, utf8_decode($row['Ciclo']), 1, 0, 'C'); // Campo 'Salida'
        $pdf->Cell(30, 7, utf8_decode($row['Total_Horas']), 1, 1, 'C'); // Campo 'Total de horas' con salto de línea
    }
    $pdf->Ln(0);

    //2da TABLA
    $sql_horas = "SELECT Id, Entrada, Salida, Actividad, Laboratorio, Encargado, `Horas` FROM vista_horas_alumnos WHERE Carnet = '$busqueda' AND Ciclo = '$ciclo'";
    ...
    $pdf->Cell(30, 7, utf8_decode('ENTRADA'), 1, 0, 'C');
    $pdf->Cell(30, 7, utf8_decode('SALIDA'), 1, 0, 'C');
    $pdf->Cell(150, 7, utf8_decode('ACTIVIDAD'), 1, 0, 'C');
    $pdf->Cell(23, 7, utf8_decode('LABORATORIO'), 1, 0, 'C');
    $pdf->Cell(25, 7, utf8_decode('ENCARGADO'), 1, 0, 'C');
    $pdf->Cell(15, 7, utf8_decode('HORAS'), 1, 0, 'C');
    $pdf->Ln(7);
    ...
    if ($result_horas->num_rows > 0) {
        ...
        while ($row = $result_horas->fetch_assoc()) {
            ...
            $pdf->Cell(30, 7, utf8_decode($row['Entrada']), 1, 0, 'C'); // Campo 'Entrada'
            $pdf->Cell(30, 7, utf8_decode($row['Salida']), 1, 0, 'C'); // Campo 'Salida'
            $pdf->Cell(150, 7, utf8_decode($row['Actividad']), 1, 0, 'L'); // Campo 'Actividad'
            $pdf->Cell(23, 7, utf8_decode($row['Laboratorio']), 1, 0, 'C'); // Campo 'Laboratorio'
            $pdf->Cell(25, 7, utf8_decode($row['Encargado']), 1, 0, 'C'); // Campo 'Encargado'
            $pdf->Cell(15, 7, utf8_decode($row['Horas']), 1, 1, 'C'); // Campo 'Total de horas' con salto de línea
        }
    }
