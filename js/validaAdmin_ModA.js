document.addEventListener("DOMContentLoaded", function() {
    ...
    var contactoE = document.getElementById('contactoE');
    var telefonoE = document.getElementById('telefonoE');
    ...
    form.addEventListener('submit', function(event) {
        ...
        var telefonoEPattern = /^\d{4}-\d{4}$/;

        if (!nombresPattern.test(nombres.value) || !apellidosPattern.test(apellidos.value)) {
            alert("Por favor ingrese un nombre o apellido válido");
        ...
        if (!telefonoPattern.test(telefono.value)) {
            alert("Ingrese un numero valido en el formato 1234-5678");
            event.preventDefault();
            return false;
        }
        ...
        if (!telefonoEPattern.test(telefonoE.value)) {
